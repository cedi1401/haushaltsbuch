...
/**
 * Fetches current market prices for a list of assets from the main process.
 * Only available in Electron. Returns null in browser mode.
 *
 * @param {Array} assets Array of asset objects with marketSymbol and marketAssetType
 * @param {string} baseCurrency ISO-4217 currency code
 */
export async function fetchMarketPrices(assets, baseCurrency) {
  if (isElectron) {
    return window.electronAPI.fetchMarketPrices(assets, baseCurrency);
  }
  return null;
}

/**
 * Triggers a full market data update (same logic as the daily scheduler).
 * Only available in Electron.
 */
export async function triggerMarketUpdate() {
  if (isElectron) {
    return window.electronAPI.triggerMarketUpdate();
  }
  return null;
}

/**
 * Invalidates all cached prices and FX rates.
 * Call this when the user changes the baseCurrency of a book.
 */
export async function invalidateMarketCaches() {
  if (isElectron) {
    return window.electronAPI.invalidateMarketCaches();
  }
}

/**
 * Registers a callback for when the scheduler completes a price update.
 * The callback receives { updatedCount, errorCount, timestamp }.
 * Only available in Electron.
 */
export function onMarketDataUpdated(callback) {
  if (isElectron) {
    window.electronAPI.onMarketDataUpdated(callback);
  }
}
